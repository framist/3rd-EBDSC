...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.):
        ...
        # - 频域
        self.fdw_real = ReparamLargeKernelConv(in_channels=dmodel, out_channels=dmodel,
                                         kernel_size=large_size, stride=1, groups=dmodel,
                                         small_kernel=small_size, small_kernel_merged=small_kernel_merged, nvars=nvars)
        self.fdw_imag = ReparamLargeKernelConv(in_channels=dmodel, out_channels=dmodel,
                                         kernel_size=large_size, stride=1, groups=dmodel,
                                         small_kernel=small_size, small_kernel_merged=small_kernel_merged, nvars=nvars)
        self.fnorm = nn.BatchNorm1d(dmodel)
        
        self.embed_size = dmodel
        self.hidden_size = dff
    ...
    def fre_DWConv_MLP(self, x: torch.Tensor):
        # embedding x: [B, M=2, D, T(N)]
        B, M, D, T = x.shape
        
        # - 转为复数 M=2 as complex number's real and imaginary part
        x = x.permute(0, 2, 3, 1)
        ...
        # - iFFT
        x = torch.fft.ifft(x, n=T, dim=2, norm="ortho")
    
        # - 转为实数
        x = torch.stack([x.real, x.imag], dim=-1)
        x = x.permute(0, 3, 1, 2) # x: [B, M=2, D, T(N)]
